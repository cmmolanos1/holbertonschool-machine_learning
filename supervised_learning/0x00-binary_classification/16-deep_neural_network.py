...
class DeepNeuralNetwork():
    ...
    def __init__(self, nx, layers):
        ...
        if len_pos != len(layers) or isinstance(arr_layers[0], np.integer) == False:
            raise TypeError("layers must be a list of positive integers")
        ...
        for l in range(self.L):
            ...
            if l == 0:
                ...
                weight = np.random.randn(layers[l], nx) * np.sqrt(2 / nx)
                self.weights[key_w] = weight
            else:
                weight = np.random.randn(layers[l], layers[l - 1]) * \
                         np.sqrt(2 / layers[l - 1])
                self.weights[key_w] = weight

            biases = np.zeros((layers[l], 1))
